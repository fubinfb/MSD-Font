...
class BaseTrainer:
    ...
    def infer_save_img(self, loader, tag, n_row):
        ...
        # print("the size of the grid_batches")
        # print(grid_batches.shape())
    ...
    def infer_save_DF(self, loader, tag, n_row):
        grid_batches = self.infer_loaderDF(self.gen_ema, loader)
        # print("the size of the grid_batches")
        # print(grid_batches.shape())
        # grid = utils.make_comparable_grid(*grid_batches[::-1], nrow=n_row)
        # self.writer.add_image(tag, grid, global_step=self.step)
    ...
    def ac_backward_without_frozen_ac(self):
        if self.aux_clf is None:
            return
        ac_loss = sum(self.ac_losses.values())
        ac_loss.backward(retain_graph=True)
    ...
    def saveCDF(self, method, save_freq=None):
        """
        Args:
            method: all / last
                all: save checkpoint by step
...
# designed for aux class for style and char
class BaseTrainerv2:
    def __init__(self, gen, disc, g_optim, d_optim, aux_clf, ac_optim, aux_clf_style, aux_clf_char,
                 writer, logger, cfg, use_ddp):
    # def __init__(self, gen, disc, g_optim, d_optim, aux_clf, ac_optim,
    #              writer, logger, cfg, use_ddp):
        self.step = 0
        ...
        self.aux_clf_style = aux_clf_style
        self.aux_clf_char = aux_clf_char
    ...
    def clear_losses(self):
        ...
        # ac losses
        loss_dic.update({k: v.item() for k, v in self.ac_losses.items()})
        loss_dic.update({k: v.item() for k, v in self.cls_losses.items()})
        loss_dic.update({k: v.item() for k, v in self.frozen_ac_losses.items()})
    ...
    def cls_backward(self):
        # if self.aux_clf_style is None:
        #     return
        cls_loss = sum(self.cls_losses.values())
...
class BaseTrainerv3:
    ...
    def clear_d_losses(self):
        """ Integrate & clear loss dict """
        # g losses
        # loss_dic = {k: v.item() for k, v in self.g_losses.items()}
        # loss_dic['g_total'] = sum(loss_dic.values())
        # d losses
        loss_dic = {k: v.item() for k, v in self.d_losses.items()}
        # ac losses
        # loss_dic.update({k: v.item() for k, v in self.ac_losses.items()})
        # loss_dic.update({k: v.item() for k, v in self.frozen_ac_losses.items()})
        # loss_dic.update({k: v.item() for k, v in self.cls_losses.items()})
    ...
    def add_ori_pixel_loss(self, out, target):
        loss = self.add_loss(
            (out, target), self.g_losses, "ori_pixel", self.cfg["pixel_w"] * 0.5, F.l1_loss
        )
